    ...
    for bin_file_data in input_dict['xml_files']:
        try:
            ...
            for host_obj in hosts_dict:
                ...
                hosts_dict[host_obj]['ccf_id'] = host_id

            # add ports
            for port_obj in ports_dict:
                current_port = db.select_host_port(hosts_dict[ports_dict[port_obj]['host_report_id']]['ccf_id'],
            ...
            if not input_dict['only_nmap']:
                ...
                for website_obj in website_row:
                    ...
                    ccf_port_id = ports_dict[service_id]['ccf_id']
                    ccf_host_id = hosts_dict[ports_dict[service_id]['host_report_id']]['ccf_id']
                    ccf_hostname_id = "0"
                    if vhost:
                        current_hostname = db.select_ip_hostname(ccf_host_id, vhost)
                        if current_hostname:
                            hostname_id = current_hostname[0]['id']
                        else:
                            hostname_id = db.insert_hostname(ccf_host_id, vhost,
                                                             input_dict['hostnames_description'],
                                                             current_user['id'])
                        ccf_hostname_id = hostname_id

                    web_dict[web_id] = {
                        'ccf_port_id': ccf_port_id,
                        'ccf_host_id': ccf_host_id,
                        'ccf_hostname_id': ccf_hostname_id
                    }
                # Add web vulns
                ...
                for vuln_obj in vuln_row:
                    ...
                    services = {
                        web_dict[vuln_website_id]['ccf_port_id']: [web_dict[vuln_website_id]['ccf_hostname_id']]}
