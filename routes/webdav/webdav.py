...
class FileObject:
    ...
    def __init__(self, file_stream, buffer=0):
        config_ccf = config_dict()
        self.max_size = int(config_ccf["files"]["files_max_size"])
        self.current_size = 0
...
class CategoryTypeCollection(DAVCollection):
    ...
    def create_empty_resource(self, filename):
        ...
        config_ccf = config_dict()
        storage = config_ccf["files"]["files_storage"]
...
class VirtualNonCollection(DAVNonCollection):
    ...
    def handle_move(self, destPath):
        ...
        if file_size > int(config_ccf['files']['files_max_size']):
            return False

        if config_ccf['files']['files_storage'] == 'filesystem':
            f = open('./static/files/code/' + file_id, 'wb')
